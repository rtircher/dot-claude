...
| Plugin        | What it does                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `conventions` | Always-on working-style rules (TDD, git hygiene, review gates, no-heredoc, model selection), injected into every session via a `SessionStart` hook and, minus the orchestration sections, into code-writing and code-judging subagents (the `dev` plugin's `coder`/`researcher`/`reviewer` and the built-in `general-purpose`) via a `SubagentStart` hook; Explore, Plan, and lens reviewers get nothing. Edit [`conventions.md`](plugins/conventions/conventions.md). |
