...
| `dev`         | On-demand software-development tooling: the `adversarial-review` and `autonomous-feature` **skills**, the `/dev:babysit` PR-shepherding loop plus the `/dev:handover` + `/dev:takeover` session-continuity **commands**, and the `coder` (worktree-isolated) + `researcher` (read-only) **agents** for parallel subagent work. |
