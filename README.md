...
| Plugin        | What it does                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `conventions` | Always-on working-style rules (TDD, git hygiene, review gates, no-heredoc, model selection), injected into every session via a `SessionStart` hook and, minus the orchestration sections, into every subagent via a `SubagentStart` hook. Edit [`conventions.md`](plugins/conventions/conventions.md). |
...
backstop, a git-identity warning, and a doctor. Every vendored artifact lives under
`.claude/cloud/`. The generic `cloud-setup.sh` also calls an OPTIONAL repo-authored
`.claude/cloud/cloud-setup-local.sh` for work that needs root at
