...
repo="$(mktemp -d)"; mkdir -p "$repo/.claude/cloud"
sentinel="$repo/ran"
printf '#!/usr/bin/env bash\n: > %q\n' "$sentinel" > "$repo/.claude/cloud/cloud-setup-local.sh"
chmod +x "$repo/.claude/cloud/cloud-setup-local.sh"
...
repo="$(mktemp -d)"; mkdir -p "$repo/.claude/cloud"
printf '#!/usr/bin/env bash\nexit 7\n' > "$repo/.claude/cloud/cloud-setup-local.sh"
chmod +x "$repo/.claude/cloud/cloud-setup-local.sh"
